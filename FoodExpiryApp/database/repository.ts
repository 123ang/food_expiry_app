...
export const CategoryRepository = {
  ...
  // Get count of food items in a category
  getItemCount: (id: number): Promise<number> => {
    return new Promise((resolve, reject) => {
      const db = getDatabase();
      db.transaction(tx => {
        tx.executeSql(
...
export const LocationRepository = {
  ...
  // Get count of food items in a location
  getItemCount: (id: number): Promise<number> => {
    return new Promise((resolve, reject) => {
      const db = getDatabase();
      db.transaction(tx => {
        tx.executeSql(
          'SELECT COUNT(*) as count FROM food_items WHERE location_id = ?',
          [id],
          (_, { rows }) => {
            resolve(rows._array[0].count);
          },
